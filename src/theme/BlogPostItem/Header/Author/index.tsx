...
import {BlogPostAuthor as BlogPostAuthorType} from '@docusaurus/plugin-content-blog';

import styles from './styles.module.css';

interface Author extends BlogPostAuthorType {
  socials?: {
    x?: string;
    linkedin?: string;
    github?: string;
...
function AuthorSocialLinks({author}: {author: Author}): JSX.Element | null {
  ...
  return (
    <div className={styles.authorSocials}>
      {socials.x && (
        <Link
          href={socials.x.startsWith('http') ? socials.x : `https://x.com/${socials.x}`}
...
export default function BlogPostItemHeaderAuthor({
  ...
  const link = url || (imageURL && `#local-author-${name}`);

  return (
    <div className={clsx('avatar', className)}>
      {imageURL && (
        <Link className="avatar__photo-link" href={link}>
